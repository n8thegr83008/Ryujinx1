...
namespace Ryujinx.HLE.HOS.Kernel.Memory
{
    class KPageHeap
    {
        private class Block
        {
            ...
            public int PagesCount => (int)(Size / KMemoryManager.PageSize);
        ...
        {
            _usedSize = _heapSize - (GetFreePagesCount() * KMemoryManager.PageSize);
        }
        ...
        public ulong AllocateBlock(int index, bool random)
        {
            ...
            for (int i = index; i < _blocksCount; i++)
            {
                ...
                if (address != 0)
                {
                    ...
                    {
                        Free(address + neededSize, (allocatedSize - neededSize) / KMemoryManager.PageSize);
                    }
        ...
        public void Free(ulong address, ulong pagesCount)
        {
            ...
            ulong start       = address;
            ulong end         = address + pagesCount * KMemoryManager.PageSize;
        ...
        {
            return GetBlockSize(index) / KMemoryManager.PageSize;
        }
        ...
        private static int CalculateManagementOverheadSize(ulong regionSize, int[] blockShifts)
        {
            ...
            return BitUtils.AlignUp(overheadSize, KMemoryManager.PageSize);
        }
    }
}
